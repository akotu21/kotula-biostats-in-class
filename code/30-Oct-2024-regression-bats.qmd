...
```{r}
bats <- read.csv(here("data", 
                      "tricolored_bat_hibernation.csv"))
glimpse(bats)
```
...
Roughly speaking, the mass lost during hibernation ranges between **0.2 and 1.9g**

Roughly speaking, the number of arousals varies between 4 and 16

Slope is rise/run, which in this case is about **1.7** for rise and 12 for run, so we get **0.1416** as our guesstimate of the slope.

Eyeballing the plot, we would also guess that the y-intercept is somewhere around 0.3
